...
img_paths = list(Path("/content/yolov5/runs/detect/exp").glob("*.jpeg"))[:9]
show_grid(img_paths, 420)

# display unannotated image from validation set, its anotated version, our model output's version
outputImg = list(Path("/content/yolov5/runs/detect/exp").glob("*.jpeg"))[1]
unannotatedImg = Path('/content/FLIR_ADAS_1_3/val/thermal_8_bit/{0}'.format(outputImg.name))
annotatedImg = Path('/content/FLIR_ADAS_1_3/val/Annotated_thermal_8_bit/{0}'.format(outputImg.name))
