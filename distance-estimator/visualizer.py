...
def write_predictions_on_frames(df):
    for idx, row in df.iterrows():
        ...
        if os.path.exists(fp):
            im = cv2.imread(fp)
            string = "(distance: {})".format(row['distance'])
            cv2.putText(im, string, x1, y1+20, cv2.FONT_HERSHEY_SIMPLEX,
                        1, (255, 255, 255), 1.5, cv2.LINE_AA)
            cv2.imwrite(fn, im)
            cv2.waitKey(0)
...
def generate_video_from_frames():
    ...
    img_array.sort()
    out = cv2.VideoWriter(
        "output.mp4", cv2.VideoWriter_fourcc(*'MP4V'), fps, size)
